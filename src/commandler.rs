...
pub fn commandler(line_read: String) {
    ...
    match command {
        ...
        _ => {
            ...
            let command_tuple = check_command_existence(command, paths.clone());
            // If command exists, run it
            if command_tuple.0 {
                let full_path_command = command_tuple.1;
                let mut child = Command::new(full_path_command).args(args).spawn().unwrap();
...
fn check_command_existence(command: &str, paths: Vec<String>) -> (bool, String) {
    ...
    if command.chars().nth(0).unwrap() == '/' {
        ...
        for single_path in paths {
            if Path::new(format!("{}/{}", single_path.clone(), command).as_str()).exists() {
